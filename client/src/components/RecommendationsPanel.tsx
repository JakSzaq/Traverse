  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="w-full relative h-screen flex flex-col justify-start items-center duration-500 overflow-hidden mt-20"
      >
        <div className="top flex w-full flex-col justify-center items-center">
          <h2 className="text-5xl py-10">Oto nasze propozycje</h2>
          <div className="list flex flex-row gap-5">
            <h3
              onClick={() => navigate("/")}
              className="group text-3xl font-bold bg-white px-10 py-2 rounded-full flex items-center justify-center gap-4 cursor-pointer"
            >
              <img
            ...
            {recommendationData[result!].places.map((place, i) => (
              <h3
                ...
                className={`text-3xl font-bold bg-primary-color px-10 py-2 cursor-pointer rounded-full border-4 border-transparent duration-100 hover:bg-opacity-50 flex  items-center justify-center ${
                  selected == i && "bg-transparent !border-primary-color"
                }`}
              >
                {recommendationData[result!].places[i].split(",")[0]}
                <p className=" absolute text-sm top-44 w-auto h-6 px-4 bg-back-color border-2 border-primary-color rounded-full">
                  {place.split(",")[1]}
                </p>
              </h3>
            ))}
        ...
        <motion.div
          ...
          className="bottom w-[65%] h-auto aspect-square absolute -bottom-3/4 rounded-full flex justify-center items-center border-8 border-primary-color bg-contain bg-no-repeat"
        ></motion.div>
      </motion.div>
    </AnimatePresence>
  );
