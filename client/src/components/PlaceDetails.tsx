...
const PlaceDetails: React.FC<PlaceDetailsI> = ({
  ...
  return (
    <>
      {place !== null && (
        <div className="flex flex-col h-full w-full gap-4 mt-4 pb-4">
          <div className="header relative h-auto flex flex-col md:flex-row items-center justify-center gap-2 md:gap-6">
            <img
              className={
                "w-10 md:absolute left-0 cursor-pointer duration-100 hover:translate-x-2"
              }
              src={BackIcon}
              onClick={() => setSelectedPlace(undefined)}
            />
            <h2
              className={`md:ml-20 font-bold ${
          ...
          <div className="content shadow-[inset_0_0_10px_rgba(0,0,0,.25)] rounded-2xl px-6 py-4 overflow-hidden flex flex-col gap-2">
            <div className="top flex flex-col md:flex-row items-center justify-start gap-6 w-full">
              <div className="address w-full max-w-sm flex flex-col items-center md:items-start duration-300 hover:max-w-full">
                <h2 className="font-bold text-2xl text-primary-color">Adres</h2>
                <h2 className="font-bold text-2xl sm:text-3xl text-black text-ellipsis overflow-hidden whitespace-nowrap">
                  {place.vicinity?.split(",")[0]}
                </h2>
              </div>
              <div className="min-w-full md:min-w-[4px] h-1 md:h-16 bg-primary-color "></div>
              <div className="w-full contact flex flex-col">
                ...
                {place.website !== undefined && (
                  <a
                    ...
                    className="group font-bold w-full relative text-2xl text-black flex flex-row gap-4 cursor-pointer overflow-hidden"
                  >
                    <img src={WebIcon} />{" "}
            ...
            <div className="description h-full w-full flex flex-col md:flex-row">
              <div className="relative photos w-full md:w-[50%] h-[19rem] py-2 md:p-2 !pl-0 flex items-end justify-center">
                <img
                  className="h-64 w-full rounded-2xl duration-100 object-cover"
                  src={String(place.photos![photo].getUrl())}
                />
                <div className="controls absolute top-0 translate-y-4 md:w-[75%] flex flex-row items-center justify-between gap-5 md:gap-10 bg-back-color px-4 py-1 border-4 border-white drop-shadow-[0_0_5px_rgba(0,0,0,.25)] rounded-full">
                  <img
                    onClick={() => changePhoto("prev")}
                    className="w-8 cursor-pointer"
                    src={BackIcon}
                  />
                  <h2 className="hidden xs:block font-bold text-2xl">
                    ZDJĘCIA
                  </h2>
                  <img
              ...
              <div className="reviews mt-4 md:mt-0 md:w-[50%] flex flex-col items-center overflow-hidden">
                <h2 className="font-bold text-primary-color mb-2 text-xl xs:text-2xl pb-0">
                  Opinie o miejscu
                </h2>
                <div className="review-list h-64 overflow-y-scroll snap-mandatory snap-y w-full">
                  {place.reviews == undefined ? (
                    ...
                    <>
                      {place.reviews?.sort(sortByRating).map((review, i) => (
                        <div
                          key={review.author_name}
                          className="review h-auto w-full mb-4 bg-back-color snap-center snap-always rounded-xl p-4 flex flex-col gap-2"
                        >
                          <div className="flex flex-col-reverse sm:flex-row justify-between items-between">
                            <div className="header flex flex-row  gap-2">
                              ...
                              />
                              <div>
                                <h2 className="font-bold w-full text-2xl max-w-[5rem] xs:max-w-[12rem] -mb-1 overflow-hidden text-ellipsis whitespace-nowrap">
                                  {review.author_name}
                                </h2>
                                <h3 className="max-w-[5rem] xs:max-w-none overflow-hidden text-ellipsis whitespace-nowrap">
                                  {review.relative_time_description}
                                </h3>
                              </div>
                            </div>
                            <div className="flex items-center justify-center gap-2 rounded-full mb-2">
                          ...
                          <h2 className="text-sm xs:text-base text-ellipsis overflow-hidden ">
                            {review.text}
                          </h2>
                        </div>
                      ))}
